...
with body_col:
    ...
    st.markdown(f"""
                ...
                <p 
                    ...
                    font-size:20px;
                >
                    Implementando <b>{dict_data['electric_vehs']} vehículos eléctricos</b> que recorran diariamente en el <b>Borough {str(dict_data['borough_name']).title()}</b> se puede reducir un <b>{dict_data['avg_daily_reduction_pct'].round(4)}%</b> de registros de sonidos de motores respecto del total de sonidos registrados relacionados a vehículos.</p>
                </div>    
                """, unsafe_allow_html=True)
